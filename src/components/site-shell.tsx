...
export function SiteShell({ locale, children }: { locale: Locale; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-canvas"><header className="fixed inset-x-0 top-0 z-50 border-b border-line bg-white/80 backdrop-blur-xl"><div className="flex h-14 items-center gap-3 px-4 sm:px-6"><Link href={prefix} className="flex h-10 w-[142px] shrink-0 items-center" aria-label="LBID home"><BrandMark markClassName="h-9 w-[138px]" /></Link><div className="hidden h-9 max-w-xl flex-1 items-center gap-2 border border-line bg-[#fbfcfe] px-3 text-sm text-ink-3 md:flex"><Search className="h-4 w-4" /><span>{t.search}</span></div><div className="ml-auto flex items-center gap-1.5">{authenticated ? <><Link href={`${prefix}/tokens`} className="hidden h-9 items-center gap-2 rounded-md border border-[#e8d9a0] bg-[#fdf8ec] px-3 text-xs font-semibold text-[#765b16] transition hover:border-[#c49a3c] hover:bg-[#fbf3dc] sm:flex"><Wallet className="h-4 w-4" />{identity?.tokens ?? 0} Token</Link><Link href={`${prefix}/notifications`} className="shell-icon-button" aria-label="Notifications"><Bell className="h-5 w-5" /></Link><CompanyIdentity locale={locale} href={`${prefix}/profile`} identity={identity} t={t} /></> : <><Button asChild variant="ghost" size="sm" className="hidden sm:inline-flex"><Link href={`${prefix}/auth`}><LogIn className="h-4 w-4" />{t.signIn}</Link></Button><Button asChild size="sm"><Link href={`${prefix}/auth?mode=register`}><UserPlus className="h-4 w-4" />{t.register}</Link></Button></>}<Button asChild variant="outline" size="sm" className="min-w-10 px-2.5"><Link href={otherHref}>{t.language}</Link></Button></div></div></header><aside className="fixed bottom-0 left-0 top-14 z-40 hidden w-[228px] border-r border-line bg-white px-3 py-5 lg:block"><NavGroup label={t.workflow} items={nav} pathname={pathname} /><NavGroup label={t.network} items={network} pathname={pathname} className="mt-8" /><NavGroup label={t.account} items={account} pathname={pathname} className="mt-8" /></aside><div className="min-h-screen pb-20 pt-14 lg:pb-0 lg:pl-[228px]">{children}</div><nav className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-4 border-t border-line bg-white/95 px-2 py-2 backdrop-blur lg:hidden">{nav.map((item) => { const active = pathname === item.href; return <Link key={item.href} href={item.href} className={`flex min-h-12 flex-col items-center justify-center gap-1 rounded-md text-[11px] font-semibold transition ${active ? "bg-[#eef1f8] text-lblue" : "text-slate-500 hover:bg-slate-50"}`}><item.icon className="h-5 w-5" /><span>{item.label}</span></Link> })}</nav></div>
}
