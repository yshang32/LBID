...
function OwnerAction({ locale, request, t }: { locale: Locale; request: ShipmentRequest; t: typeof copy.zh }) { if (request.status === "CLOSED") return <><p className="font-semibold text-lblue">{t.compare}</p><Button asChild className="mt-4 w-full"><Link href={`/${locale}/quotations/compare?srId=${request.id}`}>{t.compare}<ArrowRight className="h-4 w-4" /></Link></Button></>; if (request.status === "PENDING_REVIEW") return <><p className="flex items-center gap-2 font-semibold text-lblue"><Clock3 className="h-4 w-4 text-lgold" />{t.review}</p><p className="mt-3 text-sm leading-6 text-slate-600">{t.reviewNote}</p></>; return <><p className="flex items-center gap-2 font-semibold text-lblue"><Clock3 className="h-4 w-4 text-lgold" />{request.status === "AWARDED" ? t.awarded : t.open}</p><p className="mt-3 text-sm leading-6 text-slate-600">{t.openNote}</p></> }
