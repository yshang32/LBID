...
export function LiveDashboardPanel({ locale, mode }: { locale: Locale; mode: Mode }) {
  ...
  if (mode === "admin") return <main className="mx-auto w-full max-w-6xl px-4 pb-24 pt-8 sm:px-6 lg:pb-10"><section className="flex flex-col gap-4 border-b border-lblue/10 pb-7 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#a17e22]">LBID admin</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-lblue sm:text-4xl">{t.admin}</h1><p className="mt-2 text-slate-600">{t.adminIntro}</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href={`${prefix}/admin/accounts`}>Accounts</Link></Button><Button asChild variant="outline"><Link href={`${prefix}/admin/shipment-requests`}>SR review</Link></Button><Button asChild><Link href={`${prefix}/admin/pending-payments`}>{t.reviewPayments}<ArrowRight className="h-4 w-4" /></Link></Button></div></section></main>
