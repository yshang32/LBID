...
import { Activity, AlertTriangle, Award, CheckCircle2, ChevronRight, CircleCheck, Clock3, FileCheck, LockKeyhole, Package, Plane, Ship, TrendingUp, Wallet } from "lucide-react"
...
function HeroOpportunity({ locale, t, request, match, reasons }: { locale: Locale; t: typeof copy.zh; request: ShipmentRequest; match?: number; reasons: string[] }) {
  ...
  const progress = Math.min(100, Math.max(0, (remaining.total / (15 * 60)) * 100))
  ...
  return <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5, ease: [0.16, 1, .3, 1] }} className={`relative overflow-hidden rounded-[20px] bg-white shadow-[0_8px_40px_rgba(0,0,0,.09),0_2px_8px_rgba(0,0,0,.05)] ${finalMinutes ? "border border-red-200" : finalWindow ? "border border-[#f0d990]" : "border border-line/80"}`}>
    <div className="absolute inset-x-0 top-0 h-[3px] overflow-hidden bg-[#e8ecf4]"><motion.div className={`h-full ${finalMinutes ? "bg-[linear-gradient(90deg,#dc2626,#ef4444)]" : "bg-[linear-gradient(90deg,#c49a3c,#e8b44a)]"}`} animate={{ width: `${finalWindow ? progress : 100}%` }} transition={{ duration: .9, ease: "linear" }} /></div>
    ...
    <div className="relative px-6 pb-8 pt-9 lg:px-8">
        ...
        <motion.div animate={finalMinutes ? { scale: [1, 1.016, 1] } : { scale: 1 }} transition={finalMinutes ? { repeat: Infinity, duration: 2.4, ease: "easeInOut" } : { duration: .4 }} className={`flex items-center gap-2 rounded-xl border px-4 py-2 ${finalMinutes ? "border-red-200 bg-red-50 text-red-800" : finalWindow ? "border-[#f0d990] bg-[#fffbf0] text-[#7a5e18]" : "border-line bg-white/80 text-ink"}`}><Clock3 className={`h-3.5 w-3.5 ${finalMinutes ? "text-red-600" : finalWindow ? "text-lgold" : "text-ink-3"}`} /><span className="font-mono text-sm font-bold tabular-nums">{remaining.label}</span><span className={`text-[11px] ${finalMinutes ? "text-red-400" : finalWindow ? "text-lgold" : "text-ink-3"}`}>{t.remaining}</span></motion.div>
      </div>
      {finalWindow && !submitted ? <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} transition={{ duration: .35, ease: [0.16, 1, .3, 1] }} className={`mb-5 flex items-center gap-3 rounded-xl border px-4 py-3 ${finalMinutes ? "border-red-200 bg-red-50 text-red-900" : "border-[#f0d990] bg-[#fffbf0] text-[#7a5e18]"}`}><AlertTriangle className={`h-4 w-4 shrink-0 ${finalMinutes ? "text-red-600" : "text-lgold"}`} strokeWidth={1.75} /><p className="text-[12.5px] font-medium leading-snug">{finalMinutes ? (locale === "zh" ? "競價窗口即將結束，請立即提交報價以保留參與資格。" : "Window closing. Submit your quote now to participate.") : (locale === "zh" ? "最後競價時段。請檢查資料後提交你的密封報價。" : "Final bidding window. Review and submit your sealed quote.")}</p></motion.div> : null}
      ...
      <div className="mt-7 grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <div><p className="text-[10px] font-bold uppercase tracking-[.09em] text-ink-3">{t.sealedQuote}</p>{submitted ? <motion.div initial={{ opacity: 0, scale: .93 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .38, ease: [0.16, 1, .3, 1] }} className="mt-3 overflow-hidden rounded-xl border border-line bg-white"><div className="border-b border-line bg-[linear-gradient(180deg,#edfaf3,#f7fdf9)] px-5 py-5 text-center"><span className="mx-auto grid h-11 w-11 place-items-center rounded-full border border-emerald/25 bg-white shadow-[0_2px_10px_rgba(26,125,74,.12)]"><CheckCircle2 className="h-5 w-5 text-emerald" /></span><p className="mt-3 text-sm font-semibold text-ink">{t.submitted}</p><p className="mt-1 text-[11.5px] text-ink-3">{t.submittedBody}</p></div><div className="divide-y divide-line-light text-[11.5px]"><p className="flex items-center justify-between px-4 py-3 text-ink-3"><span>{t.amount}</span><strong className="text-[13px] text-ink">HKD {Number(amount).toLocaleString("en-HK", { minimumFractionDigits: 2 })}</strong></p><p className="flex items-center gap-1.5 px-4 py-3 text-lblue"><FileCheck className="h-3.5 w-3.5" />{locale === "zh" ? "密封報價收據已建立" : "Sealed quote receipt created"}</p></div></motion.div> : <><div className="relative mt-3 rounded-xl border-2 border-line bg-white transition duration-200 focus-within:border-lblue focus-within:shadow-[0_0_0_3px_rgba(12,26,62,.08)]"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[13px] font-semibold text-ink-3">{t.amount}</span><input value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" type="number" min="0" placeholder="0.00" aria-label={t.sealedQuote} className="w-full rounded-xl bg-transparent py-4 pl-[52px] pr-4 text-[22px] font-semibold tracking-[-.3px] text-ink outline-none placeholder:text-line [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" /></div><Button onClick={submitQuote} disabled={submitting || remaining.total <= 0} className={`mt-3 w-full rounded-xl py-3.5 shadow-[0_4px_16px_rgba(12,26,62,.24)] hover:-translate-y-px ${finalMinutes ? "bg-[linear-gradient(135deg,#3b0a0a,#991b1b)] hover:bg-red-900" : ""}`}>{submitting ? t.submitting : t.submit}</Button>{error ? <p className="mt-2 text-xs text-red-600">{error}</p> : null}<p className="mt-3 text-center text-[10.5px] text-ink-3"><Link className="hover:text-lblue hover:underline" href={`/${locale}/marketplace/${request.id}`}>{t.details}</Link></p></>}</div>
      </div>
    </div>
  </motion.section>
}
