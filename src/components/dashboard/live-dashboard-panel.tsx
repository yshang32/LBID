...
function WorkspaceHero({ locale, t, request, href, match, reasons }: { locale: Locale; t: typeof copy.zh; request: Request; href: string; match?: number; reasons: string[] }) {
  const routeText = route(request, locale)
  const cargoText = cargo(request)
  const shownReasons = reasons.length ? reasons : [locale === "zh" ? "公司服務能力與需求相符" : "Your company capability matches this request", locale === "zh" ? "這是公開的密封競價機會" : "This is a fair sealed-bid opportunity"]
  return <section className="relative mt-7 overflow-hidden rounded-lg border border-line bg-white shadow-[0_8px_40px_rgba(12,26,62,.09)]"><div className="absolute inset-x-0 top-0 h-[3px] bg-[linear-gradient(90deg,#0c1a3e_0%,#1e3a7a_54%,#c49a3c_100%)]" /><div className="relative grid gap-8 px-6 pb-7 pt-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-8"><div><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-2 text-[11px] font-bold tracking-[.09em] text-[#9a7520]"><span className="h-1.5 w-1.5 rounded-full bg-lgold" />{match ? t.recommended : t.open}</span>{match ? <span className="rounded-full border border-[#e8d9a0] bg-[#fdf8ec] px-3 py-1 text-[11px] font-semibold text-[#7a5e18]">{match}% {t.match}</span> : null}</div><h2 className="mt-6 text-[clamp(1.7rem,2.6vw,2.4rem)] font-bold tracking-[-.035em] text-ink">{routeText}</h2><p className="mt-2 text-sm text-ink-2">{cargoText}</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><div><p className="lbid-eyebrow">{t.why}</p><div className="mt-3 space-y-2">{shownReasons.map((reason) => <p key={reason} className="flex items-start gap-2 text-sm leading-5 text-ink-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald" />{reason}</p>)}</div></div><div className="rounded-lg border border-line bg-[#f8f9fc] p-4"><LockKeyhole className="h-4 w-4 text-lblue" /><p className="mt-2 text-xs leading-5 text-ink-2">{t.private}</p></div></div></div><div className="flex flex-col justify-between rounded-lg bg-[linear-gradient(135deg,#0c1a3e,#1a3a7a)] p-5 text-white shadow-[0_10px_28px_rgba(12,26,62,.24)]"><div><p className="text-[11px] font-semibold tracking-[.1em] text-[#f0d782]">{t.open}</p><div className="mt-5 flex items-center gap-4"><div className="grid h-20 w-20 place-items-center rounded-full border border-[#e8d37b] bg-[radial-gradient(circle,rgba(196,154,60,.3),rgba(196,154,60,.04)_68%)]"><div className="text-center"><p className="text-2xl font-bold leading-none">{match || "--"}{match ? "%" : ""}</p><p className="mt-1 text-[9px] font-semibold tracking-[.08em] text-[#d9e4fb]">MATCH</p></div></div><div><p className="text-[10px] font-semibold tracking-[.09em] text-[#bfcce7]">SEALED BID</p><p className="mt-1 text-sm leading-5 text-[#e7edf9]">{locale === "zh" ? "提交一次專業報價，讓 Agency 於關標後公平比較。" : "Submit one professional quote for a fair post-close comparison."}</p></div></div></div><Button asChild variant="gold" className="mt-7 w-full"><Link href={href}>{locale === "zh" ? "查看並提交報價" : "Review and bid"}<ArrowRight className="h-4 w-4" /></Link></Button></div></div></section>
}
