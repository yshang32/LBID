...
begin
  ...
  if v_sr.status = 'AWARDED' then
    raise exception 'SR_ALREADY_AWARDED';
  end if;
  ...
  values (
    ...
    v_bid.forwarder_id,
    v_line_items,
    coalesce(p_total_amount, v_bid.price),
    v_public_token,
    'accepted'
  )
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'currency', v_bid.currency,
      'route', v_sr.route,
      'services_needed', v_sr.services_needed,
  ...
  return jsonb_build_object(
    ...
    'quotation', jsonb_build_object(
      'id', v_quotation.id,
      'shipment_request_id', v_quotation.shipment_request_id,
      'forwarder_id', v_quotation.forwarder_id,
      'line_items', v_quotation.line_items,
      'total_amount', v_quotation.total_amount,
    ...
    'match_record', jsonb_build_object(
      'id', v_match.id,
      'shipment_request_id', v_match.shipment_request_id,
      'agent_id', v_match.agent_id,
      'forwarder_id', v_match.forwarder_id,
      'winning_quotation_id', v_match.winning_quotation_id,
      'stage', v_match.stage,
      'contact_revealed_at', v_match.contact_revealed_at,
